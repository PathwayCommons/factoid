...
window.cyutil = {
  ...
  relayout: function( next ){
    var canLayout = cy.nodes().length !== 0;
    if( !canLayout ){ return }

    if( next ){
      cy.one('layoutstop', next);
    }
    ...
    cy
      ...
      .layout({
        ...
        stableEnergy: function( energy ){
          var e = energy; 
          return (e.max <= 0.4) || (e.mean <= 0.4);
        }
      })
    ;
  },

  drawMode: function(on){
    if( on ){
      $('#graph').cytoscapeEdgehandles('drawon');
    } else {
      $('#graph').cytoscapeEdgehandles('drawoff');
    }
  },
