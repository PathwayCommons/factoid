$(function(){
  ...
  $('.help-ball').each(function(){
    ...
    $.helperQtip({
      ...
      ignoreForClose: '.overlay, #extendo-info'
    }, {
      position: {
        my: 'top right',
        at: 'bottom center'
  ...
  if( doc.getStage() === 'textmining' ){
    var initSplashClose = true;
    doc.hideOverlay('about', function(){ // shown only after splash close
      if( initSplashClose ){
        $('.help-ball[data-stage="textmining"]').trigger('showqtip');
        initSplashClose = false;
      }
    });
  }

  // keep track of help tooltips killed by splash
  var $lastSplashHelpBall;
  doc.showOverlay('about', function(){
    var $qtip = $('.help-ball-qtip:visible');
    var stage = $qtip.find('.help-content').attr('data-for-stage');
    var $ball = $('.help-ball[data-stage="'+ stage +'"]');

    if( $qtip.length > 0 ){
      $lastSplashHelpBall = $ball;
    } else {
      $lastSplashHelpBall = null;
    }
  });

  // restore help qtips from splash
  doc.hideOverlay('about', function(){ 
    if( $lastSplashHelpBall ){
      $lastSplashHelpBall.trigger('showqtip');
    }
  });
