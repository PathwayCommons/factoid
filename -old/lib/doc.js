  ...
  return doc = {
    ...
    removeEntity: function( entityId ){ 
      ...
      if( isFunction( fn ) ){ // then bind to when an entity is removed from the doc
        ...
        this.removeEntities( [ entityId ] );
      }
    },

    // a more efficient version for removing multiple entities at once
    removeEntities: function( entityIds ){
      ...
      // remove references to the removed entities in all interactions (& hanging intns)
      for( var i = 0; i < ints.length; i++ ){
        ...
        for( var j = pids.length - 1; j >= 0; j-- ){
          ...
          for( var k = 0; k < entityIds.length; k++ ){
            ...
            if( pid === entityId ){
              affectedIntnIds.push( intn.id ); // keep track of interactions who have had their pids plucked
              model.remove('_page.doc.entities.' + intnId + '.participantIds', j, 1);
            }
          } // for ids
      ...
      // actually remove the ents (done in batch for speed)
      model.set('_page.entIdsToDelete', entityIds);
      var entsReflist = model.refList('_page.entsToDelete', '_page.doc.entities', '_page.entIdsToDelete', { deleteRemoved: true });
      model.remove('_page.entsToDelete', 0, entityIds.length);
      model.removeRef('_page.entsToDelete');
      model.removeRef('_page.entIdsToDelete');

      // remove hanging intns
      var hangingIntns = [];
      for( var i = 0; i < affectedIntnIds.length; i++ ){
        ...
        var hanging = pids && pids.length <= 1;

        if( hanging ){
          hangingIntns.push( intnId );
        }
      }
      this.removeEntities( hangingIntns );
    },
    ...
    getAssociatedEntitiesFromQuery: function(entityId, query, ok ){
      var mLastT = model.at('_page.lastPotentialAssociationTime.' + entityId);
      ...
      var mLastQ = model.at('_page.lastPotentialAssociationQuery.' + entityId);

      if( !query || query.match(/$\s*^/) ){ // then no results
        mPotls.set([]);
        mLoading.set(false);
        return;
      ...
      var sameQAsLast = mLastQ.get() === query;
      var potls = mPotls.get();
      if( sameQAsLast ){ // then no need to search again
        mLoading.set(false);
        return;
      } else {
        mLastQ.set( query );
      ...
      services.getAssociatedEntitiesFromQuery( (atLeastOneOrg ? queryObj : query), function(err, entities){
        var thisQueryIsTheLatestOne = thisChangeTime >= mLastT.get();
    ...
    setTextminingText: function( text ){
      model.set('_page.textminingText', text);
    },

    getTextminingText: function(){
      return model.get('_page.textminingText');
    },
    ...
    addEntitiesFromText: function(text, posFn, next){
      ...
      services.getPotentialEntitiesFromText(text, function(err, ents, orgIds){
        ...
        for( var i = 0; i < ents.length; i++ ){
          ...
          // set interactions creation time to earlier so they're sorted after ents when added in batch
          if( ent.interaction ){
            model.set( '_page.doc.entities.' + ent.id + '.creationTimestamp', startTime );
          } else { // kick off an initial entity association search so it's cached
           doc.getAssociatedEntitiesFromQuery( ent.id, ent.name );
          }
        ...
        if( ents.length > 0 ){
          ...
          if( stage === 'textmining' ){
            doc.goToStage('organisms');
          }

          doc.completeStage('textmining');
        }

        next && next( err, ents.length );
      });
    },
    ...
    progressOrganismStage: function(){
      ...
      if( stage === 'organisms' ){
        this.goToStage('entities');
      }

      this.completeStage('organisms');
    },
    ...
    goToStage: function( stage, complete ){
      var fn = arguments[0];

      if( stage && complete ){
        var prevStage = this.getStage();

        this.completeStage( prevStage );
      }
    ...
    completeStage: function( stage ){
      model.at('_page.doc.stageComplete').at( stage ).set( true );
    },
    ...
    showOverlay: function( id, listener ){
      if( isFunction(id) ){
        listener = id; 
      }
      ...
      if( isFunction(listener) ){
        if( id !== listener ){
          model.on('change', '_page.showOverlay.' + id, function( shown ){
            if( shown ){ listener(shown, id); }
          });
        } else {
          model.on('change', '_page.showOverlay.*', function( id, shown ){
            if( shown ){ listener(shown, id); }
          });
        }
    ...
    hideOverlay: function( id, listener ){
      if( isFunction(id) ){
        listener = id; 
      }
