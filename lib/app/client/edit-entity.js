$(function(){
  ...
  $('body').on('keydown keyup keypress', '.edit-name-input', _.debounce(function(){
    ...
    var lastName = $input.attr('data-last-value');
    $input.attr('data-last-value', name);

    if( lastName !== name ){
      doc.getAssociatedEntitiesFromQuery(entityId, name, function(){
        var thisQueryIsTheLatestOne = thisChangeTime >= lastChangeTime;
        return thisQueryIsTheLatestOne; // ignore if the query is old
      });
    }
  }, 250));
