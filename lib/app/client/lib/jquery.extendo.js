/*! jQuery Extendo - v1.0.2 - 2013-04-04
* https://github.com/maxkfranz/jquery.extendo
* Copyright (c) 2013 Max Franz; Licensed LGPL */
(function( $ ){
	...
	if( $.mobile ){
		var events = {
			tap: event("vclick"),
			tapstart: event("vmousedown"),
			tapend: event("vmouseup")
		};
	} else {
		var events = {
			...
			return $(this).each(function(){
				...
				if ( !pluginInited ) {
					...
					$.each(options.items, function(i, item){
						...
						var $item = $('<a />', $.extend( attrs, flatten(item.attrs) ))
							.data( "extendo", item )
							.attr('target', '_blank')
