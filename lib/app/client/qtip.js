...
$(function(){
  ...
  // hack for fixing qtip positions w/ ipad keyboard scroll bug
  // would not be a problem if we didn't change keyboard focus and instead left it up to the user
...
$.helperQtip = function( options, qtipOptions ){
  ...
  $(document).on( 'tap', function(e){ 
    var $tgt = $(e.target);
    var $tgts = $tgt.parents().add( $tgt );
    var tgtInsideQtip = $tgts.filter('#' + id).length > 0;
    var tgtIsActualTgt = $tgts.filter( options.selector ).length > 0;
    var tgtShouldBeIgnored = $tgts.filter( options.ignoreForClose ).length > 0;
