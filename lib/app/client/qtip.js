...
$.helperQtip = function( options, qtipOptions ){
  ...
  function isLocked(){
    return $helper.attr('data-locked') !== undefined;
  }

  $(document).on( 'lockqtip', options.selector, function(e){ 
    $helper.attr('data-locked', 'true');
  } );

  $(document).on( 'unlockqtip', options.selector, function(e){ 
    $helper.removeAttr('data-locked');
  } );
  ...
  $(document).on( 'tap', function(e){ 
    ...
    if( !tgtInsideQtip && !tgtIsActualTgt && !isLocked() ){
      shown = false;
      $helper.qtip('hide');
      // console.log('bg hide')
