// this file is responsible for proxying calls from the local window.textmining
...
var tmapi = io.connect( window.location.origin + '/tmapi' );
...
function callFn(name, args, next){
  ...
  tmapi.emit(name, id, args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8], args[9]);
}

window.textmining = {

  getAssociatedEntitiesFromQuery: function(query, next){
    callFn('getAssociatedEntitiesFromQuery', [query], next);
  },

  getPotentialEntitiesFromText: function(text, next){
    callFn('getPotentialEntitiesFromText', [text], next);
  },

  getEntityInfo: function(entity, next){
    callFn('getEntityInfo', [entity], next);
  }
