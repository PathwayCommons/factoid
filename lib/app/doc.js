...
module.exports = function( model ){
  ...
  */
  ...
  return {
    ...
    associateEntity: function( entityId, associationMetadata ){
      var fn = arguments[0];

      if( isFunction(fn) ){
        model.on('set', '_entities.*.association', function(index, meta){
          var id = model.get('_entities.' + index + '.id');
          fn( id, meta );
        });
        ...
        var assoc = {
          db: associationMetadata.db,
          dbId: associationMetadata.dbId
        };

        model.set('_entities.' + index + '.association', assoc);
        //model.set('_associationMetadataFor.' + entityId + );
