  ...
  return {
    ...
    addEntity: function( entity ){
      ...
      if( isFunction( fn ) ){ // then bind to when an entity is added to the doc
        model.on('push', '_entities', function(entity){
          fn( entity );
        });
        return;
    ...
    entityName: function( entityId, name ){
      ...
      if( isFunction(fn) ){
        ...
        mEntity.set('_changedName', true);
      }
    },

    entityHasChangedName: function( entityId ){
      return model.get('entities.' + entityId + '._changedName') ? true : false;
    },
    ...
    toggleAddParticipants: function( interactionId ){
      popoversOpen = true;
      var mInt = model.at('entities').at(interactionId);

      var shown = mInt.get('_showAddParticipants');

      if( !shown ){
        doc.closeAllPopovers();

        mInt.set('_showAddParticipants', true);

        mInt.set('_showAddParticipantsAnimated', true);
        setTimeout(function(){
          mInt.set('_showAddParticipantsAnimated', false);
        }, 250);
      
      } else {
        mInt.set('_showAddParticipants', false);
      }
    },
    ...
    loadAssociatedInfoForEntity: function( entityId, next ){
      ...
      if( shouldLoad ){
        ...
        textmining.getEntityInfo(assoc, function(err, info){
    ...
    textminingTextIsEmpty: function(){
      var text = model.get('_textminingText');
    ...
    setTextminingEntities: function( entities ){
      if( isFunction(entities) ){
        var fn = entities;

        model.on('set', '_textminingEntities', function(){
          var ents = model.get('_textminingEntities');
          var entityCount = ents == null ? 0 : ents.length;
    ...
    getTextminingEntitiesCount: function(){
      var ents = model.get('_textminingEntities');
    ...
    removeTextminingEntity: function( entityId ){
      ...
      // remove the entity from any participant lists
      entities = model.get('_textminingEntities') || [];
      for( var i = 0; i < entities.length; i++ ){
        var ent = entities[i];

        // look for pids to remove
        var pids = ent.participantIds;
        if( pids ){ for( var j = 0; j < pids.length; j++ ){
          var pid = pids[j];

          if( pid === entityId ){
             model.remove('_textminingEntities.' + i + '.participantIds', j);
             break;
          }
        } } // if & for
    ...
    haveTextminingEntities: function(){
      var ents =  model.get('_textminingEntities');
