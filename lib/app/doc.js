  ...
  */
  ...
  return {
    ...
    entityName: function( entityId, name ){
      ...
      if( isFunction(fn) ){
        emitter.on('entityName', fn); 
      } else if( name === undefined ) {
        return model.get('_entities.' + getEntityIndex(entityId) + '.name');
      } else {
        model.get('_entities.' + getEntityIndex(entityId) + '.name', name);
        emitter.emit('entityName', entityId, name);
      }
    },
    ...
    entityViewport: function( entityId, viewport ){
      ...
      if( isFunction(fn) ){
        emitter.on('entityViewport', fn); 

      } else if( name === undefined ) {
        return model.get('_entities.' + getEntityIndex(entityId) + '.viewport');
      
      } else {
        model.get('_entities.' + getEntityIndex(entityId) + '.viewport', viewport);
        emitter.emit('entityViewport', entityId, viewport);
      }
    },
