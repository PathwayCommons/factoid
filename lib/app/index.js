...
get('/', function(page, model, params) {
  ...
  return model.subscribe(docPath, function(err, doc){
    ...
    model.set('_potentialAssociationsFor', {});
    model.fn('_populatedEntities', '_entities', '_potentialAssociationsFor', function(entities, potAssocsFor){
      var popd = [];
      ...
      if( entities ){
        ...
        for( var i = 0; i < entities.length; i++ ){ 
          ...
          for( var j in entities[i] ){ // fastest way to copy an obj, unfortunately
          ...
          // create the list of potential associations
          var potAssocs = potAssocsFor[ entity.id ];
          if( potAssocs ){
            entity.potentialAssociations = potAssocs;
          } else {
            entity.potentialAssociations = [];
          }

          // populate the participants for each interaction
