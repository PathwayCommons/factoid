  ...
  return doc = {
    ...
    subscribe: function (docId, next){
      ...
      return model.subscribe(docPath, 'organisms', function(err){
        ...
        var entFilter = model.filter('documents.' + docId + '.entities', function(item){
          return true; // filter gets all entities
        });
    ...
    addEntity: function( entity ){
      ...
      if( isFunction( fn ) ){ // then bind to when an entity is added to the doc
        model.on('insert', '_entities', function(entity){
          fn( entity );
        });
        return;
    ...
    removeEntity: function( entityId ){ 
      ...
      if( isFunction( fn ) ){ // then bind to when an entity is removed from the doc
        model.on('remove', '_entities', function(index, removed, passed){
        ...
        // remove the entity
        model.del('entities.' + entityId);

        // TODO remove interactions that are hanging from deleting this entity

        // TODO remove references to the entity in interactions
      }
    },
    ...
    connectEntityToInteraction: function( entityId, interactionId ){
      ...
      if( isFunction(fn) ){ // then listen to when entities are connected to interactions
        model.on('insert', '_entities.*.participantIds', function( interactionId, index, values, passed ){
          var interactionId = model.get('_entities.' + index + '.id');
    ...
    disconnectEntityFromInteraction: function( entityId, interactionId ){
      ...
      if( isFunction(fn) ){ // then listen to when entities are disconnected to interactions
        model.on('remove', '_entities.*.participantIds', function(intIndex, index, removed){
    ...
    // call to init listening to entity name changes
    // NB should be called client side only
    keepTrackOfNameChanges: function(){
      // keep track of whether an entity's name has been changed
      model.on('change', 'entities.*.name', function(id, name, oldName){
    ...
    togglePopover: function( model, showVar, next ){
    ...
    hidePopover: function( model, showVar, next ){
      popoversOpen = false;
    ...
    showPopover: function( model, showVar, next ){
      popoversOpen = true;
    ...
    toggleAddParticipants: function( interactionId, next ){
      togglePopover( interactionId + '.showAddParticipants', next );
    },
