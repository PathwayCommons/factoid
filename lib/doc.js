...
module.exports = function( model, srvs ){
  ...
  return doc = {
    ...
    addInitListeners: function(){
      ...
      function setupParticipantsRef( entId ){
        model.refList(
          ...
          'documents.' + docId + '.entities.' + entId + '.participantIds',
          { deleteRemoved: false }
        );
      }
      ...
      // set up participants list for newly added connections
      // TODO it shouldn't be necessary to setup the ref on each connection event (should be only once on add interaction)
      this.connectEntityToInteraction(function(entId, intId){
        setupParticipantsRef( intId );
      });
      this.disconnectEntityFromInteraction(function(entId, intId){
        setupParticipantsRef( intId );
      });
    ...
    recalculatePotentialParticipants: function( interactionId ){
      ...
      model.refList(
        '_page.potentialParticipants',
        '_page.doc.entities',
        '_page.potentialParticipantIds',
        { deleteRemoved: false } // delete from id list does not mean delete entity itself
      );
    },
