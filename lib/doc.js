...
module.exports = function( model, srvs ){
  ...
  return doc = {
    ...
    removeEntity: function( entityId ){ 
      ...
      if( isFunction( fn ) ){ // then bind to when an entity is removed from the doc
        ...
        for( var i = 0; i < ints.length; i++ ){
          ...
          // if we are to remove all participants, then just remove the interaction altogether
          if( indicesToRemove.length === pids.length ){
            this.removeEntity( intnId );

          // otherwise, just remove the participants
          } else {
    ...
    toggleAnnotations: function( entId, next ){
      this.togglePopover( 'showAnnotations.' + entId, next );
    },
