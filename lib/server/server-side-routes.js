...
module.exports = function( expressApp, io ){
  ...
  expressApp.all('/ext-services/add-doc', function(req, res){
    ...
    doc.subscribe(docId, function(){
      model.set('documents.' + docId + '.needsInitialLayout', true);
      
      doc.addEntitiesFromText(text, posFn, function(err){
        res.send({ err: err, docId: docId }); // send the id of the pop'd doc
      });
    });
