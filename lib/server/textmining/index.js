...
function getPotentialEntitiesFromText( text, next ){
  getMatchesFromText(text, function(err, matches){ // get the matches
    if( err ){ // then we can't do anything
      ...
      for( var i = 0; i < matches.length; i++ ){ // for each entity
        var match = matches[i];
        var query = match.literal;

        getPotentialEntitiesFromQuery(query, function(err, ents){
          Array.prototype.push.apply( entities, ents ); // add all of ents to entities array
          matchesComplete++;

          var allMatchesDone = matchesComplete === matches.length;
          if( allMatchesDone ){
            next(err, entities);
          }
        });
